...
const RegionTable = ({ data }) => {
  ...
  const handleDelete = async (id) => {
    try {
      await deleteInternationalRegion(id);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const docRef = await addInternationalRegion(
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size="middle">
          ...
          <Button onClick={() => navigate(`/admin/international/region/${record.id}`)} type="dashed">
            Edit Destinations
          </Button>
        </Space>
      ),
