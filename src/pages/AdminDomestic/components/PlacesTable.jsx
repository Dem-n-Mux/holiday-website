...
const PlacesTable = ({ data, regionId }) => {
  ...
  const handleDelete = async (id, colRef) => {
    try {
      await deleteDomesticPlace(id, regionId, colRef);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEditMode && currentPlace) {
        ...
        const addedPlace = await addDomesticPlace(
          regionId,
      ...
      render: (text, record) => (
        <div className="flex flex-col gap-2">
          ...
          <Button
            ...
            onClick={() =>
              navigate(
                `/admin/domestic/package/${regionId}/${record.colRef}/${record.id}`
              )
            }
          >
            Edit Packages
